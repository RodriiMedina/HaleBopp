...
UserName::UserName()
{
	_texture.loadFromFile("NombreAIngresar.png");
...
void UserName::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
...
void UserName::update() {
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Enter) && RelojTecla.getElapsedTime() > debounceTime) {
		//datos.setPerfil(userName);
		bool aux = archi.buscarRegistro(userName);
			if (aux == true) {
			std::cout << "YA EXISTE UN PERFIL CON ESE NOMBRE" << std::endl;
			Director::getInstance().changeLevel(5, 0, 0, userName);
			}
		Director::getInstance().changeLevel(1, 0, 0, userName);	
	}
